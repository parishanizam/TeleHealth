...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{p{1.5cm} p{1cm} p{3.5cm} X}
      ...
      \end{itemize} \\
      03/24/25 & 2.21 & Mitchell Weingust & Implemented TA Feedback: 
      \begin{itemize}[leftmargin=*]
...
\begin{enumerate}[label={UH-LI\arabic*}. ]
  ...
  \item The system shall include a user-guided tutorial for additional usage information.\\
  \textbf{Rationale: }User tutorials can be helpful for troubleshooting and maintenance should problems arise. This tutorial is optional and not required reading for the user's interactions with the system.\\
  \textbf{Fit criterion: }User tutorials will be provided on the team's GitHub upon project completion.
\end{enumerate}
...
\begin{table}[h!]
  ...
  \cline{2-3}
   ...
   & EXPO Demonstration & Apr 8, 2025 \\ 
  \cline{2-3}
   & Final Documentation (Problem Statement, POC plan, Development Plan, Requirements Doc, Hazard Analysis, Design Document, V\&V Plan, V\&V Report, DevGuide, Source Code) & Apr 2, 2024 \\ 
  \hline
  \end{tabular}
\end{table}
